...
import { Plus, Edit, Trash2, X, Search, RefreshCw, AlertCircle, CheckCircle, UserCog, FileSpreadsheet, Upload, Shield, UserCheck, Square, CheckSquare } from 'lucide-react';
import AiAssistantButton from '@/imports/AiAssistantButton-4-13343';
import * as nhanVienApi from '@/api/admin/nhan-vien.api';
import type { AvailableUser, BatchDeleteResult } from '@/api/admin/nhan-vien.api';
...
export default function AdminStaffPage() {
    ...
    const [items, setItems] = useState<nhanVienApi.NhanVienResponse[]>([]);
    ...
    const [selected, setSelected] = useState<nhanVienApi.NhanVienResponse | null>(null);
    const [availableUsers, setAvailableUsers] = useState<AvailableUser[]>([]);
    const [assignSearch, setAssignSearch] = useState('');

    const [form, setForm] = useState({
        maNhanVien: '', ngayNhanViec: '', ngayNghiViec: '',
    ...
    const defaultAddForm = {
        ...
        maNhanVien: '', ngayNhanViec: '', ngayNghiViec: '', usersId: '',
    };
    ...
    const [importResult, setImportResult] = useState<nhanVienApi.ExcelImportResult | null>(null);
    ...
    const fetchAll = useCallback(async () => {
        try {
            setLoading(true);
            const nhanVienData = await nhanVienApi.getAllNhanVien();
            setItems(nhanVienData || []);
        } catch (err) {
            console.error(err);
    ...
    const fetchAvailableUsers = useCallback(async () => {
        try {
            const data = await nhanVienApi.getAvailableUsers();
    ...
    const filtered = useMemo(() => items.filter(i =>
        !debouncedSearch ||
        i.maNhanVien?.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
        i.tenNhanVien?.toLowerCase().includes(debouncedSearch.toLowerCase())
    ), [items, debouncedSearch]);

    const activeCount = useMemo(() => items.filter(i => !i.ngayNghiViec).length, [items]);
    const inactiveCount = useMemo(() => items.filter(i => i.ngayNghiViec).length, [items]);
    ...
    const parseDateForInput = (dateStr: string | null) => {
        if (!dateStr) return '';
        // Response format: dd/MM/yyyy → yyyy-MM-dd for <input type="date">
        const parts = dateStr.split('/');
        if (parts.length === 3) return `${parts[2]}-${parts[1]}-${parts[0]}`;
        return dateStr.split('T')[0];
    };

    const openEdit = (item: nhanVienApi.NhanVienResponse) => {
        setSelected(item);
        setEditError(null);
        setForm({
            maNhanVien: item.maNhanVien || '',
            ngayNhanViec: parseDateForInput(item.ngayNhanViec),
            ngayNghiViec: parseDateForInput(item.ngayNghiViec),
    ...
    const openAssign = async (item: nhanVienApi.NhanVienResponse) => {
        setSelected(item);
        setAssignSearch('');
    ...
    const openDelete = (item: nhanVienApi.NhanVienResponse) => { setSelected(item); setIsDeleteOpen(true); };
    ...
    const handleCreate = async (e: FormEvent) => {
        ...
        try {
            ...
            const nhanVienDetails = {
                maNhanVien: addForm.maNhanVien,
                ngayNhanViec: addForm.ngayNhanViec ? `${addForm.ngayNhanViec}T00:00:00` : undefined,
                ngayNghiViec: addForm.ngayNghiViec ? `${addForm.ngayNghiViec}T00:00:00` : undefined,
            };

            if (addMode === 'existing-user') {
                await nhanVienApi.createNhanVienSimple({
                    ...nhanVienDetails,
                    usersId: addForm.usersId,
                });
                addToast('Thêm nhân viên với tài khoản có sẵn thành công', 'success');
            } else {
                await nhanVienApi.createNhanVien({
                    userDetails: {
                        hoTen: addForm.hoTen,
                ...
                addToast('Thêm nhân viên và tài khoản thành công', 'success');
            }
            setIsAddOpen(false);
            await fetchAll();
        } catch (err) {
            console.error(err);
            setAddError(extractError(err, 'Lỗi khi thêm nhân viên'));
        } finally {
    ...
    const handleUpdate = async (e: FormEvent) => {
        ...
        try {
            setIsSubmitting(true);
            await nhanVienApi.updateNhanVien(selected.id, {
                maNhanVien: form.maNhanVien,
                ngayNhanViec: form.ngayNhanViec ? `${form.ngayNhanViec}T00:00:00` : undefined,
                ngayNghiViec: form.ngayNghiViec ? `${form.ngayNghiViec}T00:00:00` : undefined,
            ...
            console.error(err);
            setEditError(extractError(err, 'Lỗi khi cập nhật nhân viên'));
        } finally {
            setIsSubmitting(false);
        }
    };
    ...
    const confirmDelete = async () => {
        if (!selected) return;
        try {
            await nhanVienApi.deleteNhanVien(selected.id);
            addToast('Xóa nhân viên thành công', 'success');
            await fetchAll();
        } catch (err) {
            console.error(err);
            addToast(extractError(err, 'Lỗi khi xóa nhân viên'), 'error');
        } finally {
    ...
    const handleAssignUser = async (usersId: string) => {
        ...
        try {
            setIsSubmitting(true);
            await nhanVienApi.assignUserToNhanVien(selected.id, usersId);
    ...
    const handleImport = async () => {
        ...
        setIsImporting(true);
        try {
            const result = await nhanVienApi.importNhanVienExcel(importFile);
    ...
    const handleBatchDelete = async () => {
        ...
        try {
            const result = await nhanVienApi.deleteNhanVienList(Array.from(selectedIds));
            setBatchDeleteResult(result);
            setSelectedIds(new Set());
            await fetchAll();
            if (result.failedCount === 0) {
                addToast(`Đã xóa ${result.deletedCount} nhân viên thành công`, 'success');
            }
        } catch (err) {
            addToast(extractError(err, 'Lỗi khi xóa nhân viên'), 'error');
        } finally {
    ...
    const filteredAssignUsers = useMemo(() => availableUsers.filter(u =>
        !assignSearch ||
                    ...
                    <div className="p-6 max-w-7xl mx-auto space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div className="bg-white border border-slate-200 rounded-xl px-5 py-3 flex items-center gap-3 shadow-sm">
                                <div className="rounded-lg bg-blue-100 p-2"><UserCog className="w-5 h-5 text-blue-600" /></div>
                                <div>
                            ...
                            <div className="bg-white border border-slate-200 rounded-xl px-5 py-3 flex items-center gap-3 shadow-sm">
                                <div className="rounded-lg bg-amber-100 p-2"><Shield className="w-5 h-5 text-amber-600" /></div>
                                <div>
                                    <p className="text-xl font-bold text-amber-700">{inactiveCount}</p>
                        ...
                        {selectedIds.size > 0 && (
                            <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-2.5 flex items-center justify-between">
                                <span className="text-sm font-medium text-red-700">
                                    Đã chọn <b>{selectedIds.size}</b> nhân viên
                                </span>
                                <div className="flex items-center gap-2">
                                    ...
                                    <table className="w-full text-sm">
                                        <thead className="sticky top-0 z-10 bg-gradient-to-r from-blue-600 to-blue-700 text-white">
                                            <tr>
                                                ...
                                                <th className="px-4 py-3 text-left font-semibold">Ngày nhận việc</th>
                                        ...
                                        <tbody className="divide-y divide-slate-100">
                                            {filtered.map(i => (
                                                <tr key={i.id} className={`transition-colors hover:bg-blue-50/60 ${selectedIds.has(i.id) ? 'bg-blue-50' : ''}`}>
                                                    ...
                                                    <td className="px-4 py-3 font-semibold text-slate-900">{i.maNhanVien}</td>
                                                    ...
                                                    <td className="px-4 py-3 text-slate-500">
                                                        {i.ngayNhanViec ? new Date(i.ngayNhanViec).toLocaleDateString('vi-VN') : '—'}
                                                    </td>
                                                    <td className="px-4 py-3">
                                                        {i.ngayNghiViec ? (
                                                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-700">Đã nghỉ</span>
                                                        ) : (
                                                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">Đang làm</span>
                                                        )}
                                                    </td>
                                                    <td className="px-4 py-3">
                                                    ...
                                                    <td colSpan={9} className="py-16 text-center">
                                                        <div className="flex flex-col items-center gap-2">
                                                            ...
                                                            <p className="font-medium text-slate-600">
                                                                {searchQuery ? 'Không tìm thấy kết quả' : 'Chưa có nhân viên nào'}
                                                            </p>
                                                            <p className="text-sm text-slate-400">
                                                                {searchQuery ? 'Thử từ khóa khác' : 'Nhấn "Thêm nhân viên" để bắt đầu'}
                                                            </p>
                                                        </div>
                                                    </td>
                                                </tr>
                        ...
                        {!loading && items.length > 0 && (
                            <div className="flex items-center justify-between text-xs text-slate-500 px-1">
                                <span>Hiển thị <b className="text-slate-700">{filtered.length}</b> / <b className="text-slate-700">{items.length}</b> nhân viên</span>
                                {searchQuery && <span>Tìm thấy {filtered.length} kết quả cho "{searchQuery}"</span>}
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={() => setIsAddOpen(false)}>
                    <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                        <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-blue-700 text-white px-6 py-4 flex items-center justify-between rounded-t-2xl z-10">
                            <h2 className="text-base font-bold flex items-center gap-2"><Plus className="w-5 h-5" /> Thêm Nhân Viên Mới</h2>
                        ...
                        <form className="p-6 space-y-5" onSubmit={handleCreate}>
                            ...
                            <div>
                                <h3 className="text-sm font-semibold text-slate-700 mb-3 pb-2 border-b border-slate-200 flex items-center gap-1.5">
                                    <Shield className="w-4 h-4 text-blue-500" /> Thông tin nhân viên
                                </h3>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className={lbl}>Mã nhân viên <span className="text-red-500">*</span></label>
                                        <input required value={addForm.maNhanVien} onChange={e => setAddForm({ ...addForm, maNhanVien: e.target.value })} placeholder="VD: NV001" className={inp} />
                                    </div>
                                    <div>
                                        <label className={lbl}>Ngày nhận việc</label>
                                        <input type="date" value={addForm.ngayNhanViec} onChange={e => setAddForm({ ...addForm, ngayNhanViec: e.target.value })} className={inp} />
                                    </div>
                                    <div>
                                        <label className={lbl}>Ngày nghỉ việc</label>
                                        <input type="date" value={addForm.ngayNghiViec} onChange={e => setAddForm({ ...addForm, ngayNghiViec: e.target.value })} className={inp} />
                                    </div>
                                </div>
                            </div>
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div className="col-span-2">
                                            ...
                                            <input value={addForm.ghiChu} onChange={e => setAddForm({ ...addForm, ghiChu: e.target.value })} placeholder="Ghi chú thêm..." className={inp} />
                                        </div>
                        ...
                        <div className="bg-gradient-to-r from-purple-600 to-purple-700 text-white px-6 py-4 flex items-center justify-between rounded-t-2xl flex-shrink-0">
                            <div>
                                <h2 className="text-base font-bold flex items-center gap-2">
                                    ...
                                    {selected.userName ? 'Gán Lại Tài Khoản' : 'Gán Tài Khoản'}
                                </h2>
                                <p className="text-xs text-purple-200 mt-0.5">{selected.maNhanVien} — {selected.tenNhanVien || 'Chưa có tên'}</p>
                            </div>
                        ...
                        <div className="p-5 flex-1 overflow-y-auto">
                            {selected.userName && (
                                <div className="flex items-center gap-2 mb-4 p-3 bg-blue-50 border border-blue-200 rounded-xl text-sm">
                                    ...
                                    <span className="text-slate-600">Tài khoản hiện tại: <span className="font-semibold text-blue-700">@{selected.userName}</span></span>
                                </div>
                            )}
                            <p className="text-sm text-slate-600 mb-4">
                                {selected.userName
                                    ? 'Chọn tài khoản chưa được gán để thay thế tài khoản hiện tại:'
                                    : 'Chọn tài khoản người dùng chưa được gán để liên kết với nhân viên này:'}
                            </p>
            ...
            {isEditOpen && selected && (
                <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={() => setIsEditOpen(false)}>
                    <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                        <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-blue-700 text-white px-6 py-4 flex items-center justify-between rounded-t-2xl z-10">
                            <div>
                                <h2 className="text-base font-bold flex items-center gap-2"><Edit className="w-5 h-5" /> Chỉnh Sửa Nhân Viên</h2>
                                <p className="text-xs text-blue-200 mt-0.5">{selected.maNhanVien} — {selected.tenNhanVien}</p>
                            </div>
                        ...
                        <form className="p-6 space-y-5" onSubmit={handleUpdate}>
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className={lbl}>Mã nhân viên <span className="text-red-500">*</span></label>
                                        <input required value={form.maNhanVien} onChange={e => setForm({ ...form, maNhanVien: e.target.value })} className={inp} />
                                    </div>
                                    <div />
                                    <div>
                                        <label className={lbl}>Ngày nhận việc</label>
                                        <input type="date" value={form.ngayNhanViec} onChange={e => setForm({ ...form, ngayNhanViec: e.target.value })} className={inp} />
                                    </div>
                                    <div>
                                        <label className={lbl}>Ngày nghỉ việc</label>
                                        <input type="date" value={form.ngayNghiViec} onChange={e => setForm({ ...form, ngayNghiViec: e.target.value })} className={inp} />
                                    </div>
                                </div>
                            </div>
                ...
                <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-[60] p-4" onClick={() => !isBatchDeleting && setIsBatchDeleteOpen(false)}>
                    <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6" onClick={e => e.stopPropagation()}>
                        ...
                        <p className="text-sm text-slate-600 mb-6">
                            Bạn có chắc muốn xóa <b>{selectedIds.size}</b> nhân viên đã chọn?
                            Những nhân viên đang có dữ liệu liên kết sẽ được báo cáo và bỏ qua.
                        </p>
            ...
            {isDeleteOpen && selected && (
                <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-[60] p-4" onClick={() => setIsDeleteOpen(false)}>
                    <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6" onClick={e => e.stopPropagation()}>
                        ...
                        <p className="text-sm text-slate-600 mb-6">
                            Bạn có chắc muốn xóa nhân viên <b>{selected.tenNhanVien || selected.maNhanVien}</b>? Hành động không thể hoàn tác.
                        </p>
                        <div className="flex gap-3">
                ...
                <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={() => setIsImportOpen(false)}>
                    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full" onClick={e => e.stopPropagation()}>
                        <div className="bg-gradient-to-r from-emerald-600 to-emerald-700 text-white px-6 py-4 flex items-center justify-between rounded-t-2xl">
                            <h2 className="text-base font-bold flex items-center gap-2"><FileSpreadsheet className="w-5 h-5" /> Import Nhân Viên từ Excel</h2>
                        ...
                        <div className="p-5 space-y-4">
                            ...
                            {importResult && importResult.message && (
                                <div className={`rounded-xl p-3 text-sm font-medium ${importResult.errorCount > 0 ? 'bg-amber-50 border border-amber-200 text-amber-700' : 'bg-green-50 border border-green-200 text-green-700'}`}>
